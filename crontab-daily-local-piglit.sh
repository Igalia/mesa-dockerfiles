...
function build_vk_gl_cts() {
    ...
    rocker build -f Rockerfile.vk-gl-cts --var VIDEO_GID=`getent group video | cut -f3 -d:` --var FPR_BRANCH="$CDLP_FPR_BRANCH" ${CDLP_GL_CTS_GTF:+--var GTF=}"$CDLP_GL_CTS_GTF" --var TAG=vk-gl-cts."$1" --var RELEASE=mesa."$CDLP_MESA_COMMIT"
    popd
